<template>
  ...
  <!-- 写博客 -->
  <el-dialog v-model="isArticlePublishEditorShow"
             fullscreen="true"
             :show-close="false"
             :modal="false">
  ...
  <!-- 编辑博客 -->
  <el-dialog v-model="isArticleUpdateEditorShow"
             fullscreen="true"
             :show-close="false"
             :modal="false">
    <template #header="{ close, titleId, titleClass }">
...
const showArticleUpdateEditorShow = (row) => {
  ...
  let articleCode = row.articleCode;
  console.log("row.articleCode:"+articleCode)
  getArticleDetail({'articleCode':articleCode}).then((e) => {
    if (e.success == true) {
...
const onSubmit = () => {
  ...
  publishArticleFormRef.value.validate((valid) => {
    ...
    form.requestMetaInfo = { traceId: "xxxxxx" };
    form.articleCode = "xxx001";
    console.log("提交内容：" + JSON.stringify(form));
    createArticle(form).then((e) => {
      console.log(e);
      if (e.code != RESPONSE_CODE_SUCCESS) {
        var message = e.msg;
        showMessage(message, "warning", "message");
        return;
      }
